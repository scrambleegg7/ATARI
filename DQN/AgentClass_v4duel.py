...
class AgentClass(object):
    ...
    def testinit(self,num_actions=3,STATE_LENGTH=3,test=False):
        ...
        tau = 0.001
        self.copyTargetQNetworkOperation = [v_t.assign(var_q[i] * tau + (1 - tau) * var_target[i] ) for i,v_t in enumerate(var_target)]
    ...
    def build_network(self,main_name="Q",STATE_LENGTH=3,reuse=False,h_size=512):
        ...
        with tf.variable_scope(main_name) as scopemain:
            ...
            with tf.variable_scope(name) as scopev4:
                ...
                b_conv4 = self.bias_variable([512],reuse)
            ...
            streamAC_shape = self.streamAC.get_shape().as_list()
            #print("stream AC shape", streamAC_shape)
            h,w,d = streamAC_shape[1],streamAC_shape[2],streamAC_shape[3]

            streamVC_shape = self.streamVC.get_shape().as_list()
            #print("stream VC shape", streamVC_shape)
            h_v, w_v, d_v = streamVC_shape[1],streamVC_shape[2],streamVC_shape[3]

            #self.streamA = slim.flatten(self.streamAC)
            #self.streamV = slim.flatten(self.streamVC)
            self.streamA= tf.reshape(self.streamAC, [-1, h * w * d])
            self.streamV= tf.reshape(self.streamVC, [-1, h_v * w_v * d_v])
            with tf.variable_scope("xavier_weights", reuse):
                xavier_init = tf.contrib.layers.xavier_initializer()
                self.AW = tf.get_variable("AW",shape=[h_size//2,self.num_actions], initializer=xavier_init)
                self.VW = tf.get_variable("VW",shape=[h_size//2,1], initializer=xavier_init)

            #self.AW = tf.Variable(xavier_init([h_size//2,self.num_actions]))
    ...
    def get_action(self,state, global_steps):
        ...
        if self.epsilon > np.random.random() or global_steps < 20000:
