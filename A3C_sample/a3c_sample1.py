...
def process(game_name, num_threads, restore,save_path):
    processes = []
    envs = []
    for i in range(num_threads+1):
        print( "Assuming ATARI game and playing with pixels",i )
    ...
    with tf.Session() as sess:
        ...
        processes.append(threading.Thread(target=evaluator, args=(agent, evaluation_env, sess, T_queue, saver, save_path,)))
...
def main(argv):
    ...
    process(game_name, num_threads=NUM_THREADS, restore=restore,save_path=save_path)
